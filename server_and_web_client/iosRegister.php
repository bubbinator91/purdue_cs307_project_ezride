<?php
    ...
    if(mysql_num_rows($result) == 0){
        $query_insert = "INSERT INTO userinfo (username, password, name, email, profile) VALUES ('" . $username . "', '" . $password . "','" . $name . "','" . $email . "','" . $profile . "')" or die('insert fail');
        mysql_query($query_insert) or die('Insert to userinfo failed');
        echo "success";

        header('Location: YOUR_DOMAIN_HERE/iosSignup.php?signup='.$username);
    }else{
